...
class NetworkInterface {
  private:
    ...
    struct waiting_frames_out_item {
        EthernetFrame frame;
        uint32_t ip{};
    };

    // 等待发送数据帧的队列
    std::list<waiting_frames_out_item> _waiting_frames_out{};

    // 等待查询MAC地址的IP地址的map，key是IP地址，value是上次查询的时间
    std::unordered_map<uint32_t, size_t> _pending_arp{};
